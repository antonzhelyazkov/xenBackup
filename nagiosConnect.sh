...
scriptName=$(basename $nagiosLog | cut -d "." -f 1)
...
if [ -f $lastRun ] && [ ! -f $nagiosLog ]; then
        ...
        if [ $(($lastRunTS + $timeNotStartedTreshold)) -lt $tsNow ]; then
                echo "WARNING Last successful run of $scriptName started on $(date -d @$lastRunTS) | running=0"
                exit $STATE_WARNING
        else
                echo "OK $scriptName finished without errors. Last successful run started on $(date -d @$lastRunTS) | running=0"
                exit $STATE_OK
        fi
fi
